...
Status: **定稿**(经两个不同模型子 agent 独立评审 + 第四轮终局 review 修正 R3 入口分类错误)
...
- **推进方案预览**:M1 证据接地(Done)→ M2 复核与独立评审(Done)→ M3 定稿与交付(当前)。
...
- **R9 · 修正错误时不得荡到反向极端(重复问题,已第二次出现)**。本 RFC 三轮演进中,同一类错误犯了两次:第一轮说"parser 无法从运行期剔除"(过弱),第二轮修正后说"parser 可整体消除"(过强),真相在中间。**规则:凡涉及"可/不可""能/不能"的能力边界断言,必须按子系统/条件分解,给出限定条件表而非单句结论**;修正一个过强/过弱断言时,必须显式检查新表述是否越到另一端。触发场景包括但不限于:代码可消除性、API 可用性、格式兼容性。
