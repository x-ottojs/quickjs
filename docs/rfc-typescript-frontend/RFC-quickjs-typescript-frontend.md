...
- **S11 · 涉及"求值时机"的语言特性,必须用真实参考编译器(tsc/babel 等)的转译输出交叉验证,不能只凭语言文档描述做实现(M6a 固化)**。M6a 实现参数装饰器时,最初按"文档描述的语义相近"假设直接复用方法装饰器的"原地求值"模式,表面测试通过(输出看起来正确),但语义有本质错误——真实 tsc 把参数装饰器表达式的求值推迟到类声明收尾(一次),而"原地求值"会导致每次构造函数调用都重复求值。这类错误对**无副作用**的真实代码不可观察(值相同),极易被"测试通过"的假象掩盖,只有用真实编译器的实际转译输出做逐行对比才能暴露。**规则:任何"求值时机/顺序"相关的实现,必须先用目标语言的真实参考实现（如 `npx typescript@latest tsc`）产出转译结果作为 oracle,再对照设计,不能只读语言规范文档做推断实现。**
