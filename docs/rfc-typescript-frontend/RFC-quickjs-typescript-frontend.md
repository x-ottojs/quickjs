...
- **S9 · TS 伪关键字识别禁止只判"当前 token 是伪关键字"就进入声明分支,必须验证后续形态(M3/M4/M5 三轮固化,阻断级 bug 的根因)**。三种不同的关键字判断陷阱,均已踩过:
  - **(a) strict-only 关键字陷阱(M3,再现于 M5)**:`interface`/`private`/`public`/`protected` 等属于 `JS_ATOM_LAST_KEYWORD` 与 `JS_ATOM_LAST_STRICT_KEYWORD` 之间的 strict-only 关键字(`update_token_ident`,`quickjs.c:22757`),在非 strict 顶层代码中不会被词法层转成对应 `TOK_*`,必须用 `token_is_pseudo_keyword`/`js_ts_is_pseudo_keyword_str` 识别;但在**总是 strict 的上下文**(如 class 体)中,它们**会**被转成专用 token(`TOK_PRIVATE` 等),此时反而必须用 `token.val == TOK_PRIVATE` 直接判断,`token_is_pseudo_keyword` 会失败。M5 的参数属性修饰符检测最初未区分这一点,踩了与 M3 相同类型的坑。
  - **(b) peek_token 词法弱化陷阱(M4)**:`enum` 是**无条件**强制关键字(atom 序号在 `JS_ATOM_LAST_KEYWORD` 之前),`next_token` 能正确识别为 `TOK_ENUM`,**但 `peek_token` 不能**——`peek_token` 用 `simple_next_token`,是只硬编码识别 `export`/`function`/`in`/`import` 等少数关键字的极简词法器,对其余关键字(含 `enum`)一律返回 `TOK_IDENT`。**规则:`peek_token` 只能用于判断是否为 `TOK_IDENT`/少数硬编码符号这类粗粒度信号,要精确判断某个具体关键字,必须用完整 trial-parse(`get_pos`+`next_token`+检查真实 `token.val`+`seek_token` 回退)。**
  - **(c) 强制关键字误判为伪关键字(M5,新发现,与 (a)(b) 均不同)**:`export` 的 atom 序号在 `JS_ATOM_LAST_KEYWORD`(`super`)**之前**,是**无条件**强制关键字(既非 strict-only,也不是纯上下文伪关键字),`s->token.val` 本身就会是 `TOK_EXPORT`。误用 `token_is_pseudo_keyword(s, JS_ATOM_export)` 检测必然失败(因为该函数只匹配 `TOK_IDENT`,`export` 从不是 `TOK_IDENT`)。**规则:新增任何关键字检测前,必须先在 `quickjs-atom.h` 里核实该 atom 相对 `JS_ATOM_LAST_KEYWORD`(`super`)与`JS_ATOM_LAST_STRICT_KEYWORD`(`yield`)的位置——在 `super` 之前=无条件强制关键字(用 `token.val==TOK_XXX`);在两者之间=strict-only(strict上下文用`TOK_XXX`,非strict上下文用伪关键字检测);无预定义 atom=纯伪关键字(如 `type`/`declare`/`namespace`/`readonly`,只能用 `js_ts_is_pseudo_keyword_str`)。不能凭直觉分类,必须查表。**
  - **(a)(b)(c) 共同要求**:无论哪种陷阱,识别关键字后**仅识别不够**,必须再验证紧跟的 token 形态确实像声明,否则会把 `declare = 5;`、`declare();`、`declare.x` 这类把关键字用作普通标识符的合法代码误判为声明语句并报错(M3 的 `declare` 分支初版即漏了这一验证,被子 agent 判 FAIL,已修复)。**每新增一个 TS 关键字检测点,必须同时写一条"该词作普通标识符使用"的回归测试**(参考 `tests/test_ts.js` 的 `declare`/`declare2` 用例)。
- **S10 · `define_var` 返回值在 `fd->is_global_var` 场景下是占位常量,不可作 `OP_get_loc`/`OP_put_loc` 操作数(M5 固化,M6a 第三次复现,升级为强制规则)**。`define_var`(`quickjs.c`)对 `JS_VAR_DEF_VAR`/`JS_VAR_DEF_LET` 等类型,若 `fd->is_global_var`(顶层脚本)或 `fd->is_eval`(顶层 eval)为真,返回固定的 `GLOBAL_VAR_OFFSET`(约 10 亿),不是可用的局部变量 slot 索引。**任何以 `define_var` 返回值作为 `fd->vars[]`/`fd->args[]` 数组下标的代码都是潜在的越界访问段错误**——M5(namespace)、M6a(装饰器隐藏变量)先后各踩过一次,M6a 是同一类错误的第三次出现(前两次已固化为规则但仍被绕过)。**强制规则:任何需要跨语句持有"变量位置"以便后续读写的机制,一律只存 `JSAtom`,一律只用 `OP_scope_get_var`/`OP_scope_put_var`(按 atom 名字访问,天然兼容全局与局部两种场景)访问,禁止把 `define_var` 返回值当数组下标使用**——除非能证明当前 `fd` 100% 不可能是顶层 eval/脚本作用域(如已知是嵌套函数体内)。
- **S11 · 涉及"求值时机"的语言特性,必须用真实参考编译器(tsc/babel 等)的转译输出交叉验证,不能只凭语言文档描述做实现(M6a 固化,已两次生效)**。M6a 实现参数装饰器时,最初按"文档描述的语义相近"假设直接复用方法装饰器的"原地求值"模式,表面测试通过(输出看起来正确),但语义有本质错误——真实 tsc 把参数装饰器表达式的求值推迟到类声明收尾(一次),而"原地求值"会导致每次构造函数调用都重复求值。这类错误对**无副作用**的真实代码不可观察(值相同),极易被"测试通过"的假象掩盖,只有用真实编译器的实际转译输出做逐行对比才能暴露。`emitDecoratorMetadata` 补做时再次生效:摸底阶段用真实 tsc 试探"隐式默认构造函数是否也生成 `design:paramtypes`",发现关键边界规则(仅显式构造函数触发,哪怕零参数;隐式默认构造函数完全不触发),若只凭直觉实现会产生错误结果。**规则:任何"求值时机/顺序/触发条件边界"相关的实现,必须先用目标语言的真实参考实现（如 `npx typescript@latest tsc`）产出转译结果作为 oracle,尤其要主动测试边界情况(零参数/无构造函数/空数组等),再对照设计,不能只读语言规范文档或凭直觉推断实现。**
- **S12 · "中间状态已被更早的代码转移/清空"是一类反复出现的时序 bug,新增跨阶段状态依赖时必须显式核实写入点与读取点之间是否有其他代码路径提前消费了该状态(M6a metadata 固化,本 RFC 第三次同类错误)**。本次具体表现:`js_ts_attach_pending_decorators` 在类字段的类型注解消费**之前**就已经把 `pending_member_decorators` 转移清空(该函数在字段分支最开头被调用),导致"若 `pending_member_decorators` 非空则记录类型分类"的判断永远为假。前两次同类错误分别是 M5 的作用域生命周期(变量在读取前已跨越 `pop_scope` 失效)、M6a 装饰器求值时机(参数装饰器表达式在收尾阶段才该求值,而非解析时立即求值)。**规则:引入"A 阶段写入状态,B 阶段读取状态"的设计时,必须完整追踪 A 和 B 之间的全部代码路径,确认没有 C 阶段提前消费或使该状态失效——不能只看 A 和 B 两点,必须看中间的完整调用链。**
