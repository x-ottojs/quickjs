...
- **S10 · `define_var` 返回值在 `fd->is_global_var` 场景下是占位常量,不可作 `OP_get_loc`/`OP_put_loc` 操作数(M5 新固化)**。`define_var`(`quickjs.c`)对 `JS_VAR_DEF_VAR` 类型,若 `fd->is_global_var` 为真(顶层脚本/eval 场景),返回固定的 `GLOBAL_VAR_OFFSET`,不是可用的局部变量 slot 索引。任何需要跨语句持有"变量位置"以便后续读写的机制(如 M5 的 namespace 声明合并),必须用 `OP_scope_get_var`/`OP_scope_put_var`(按 atom 名字访问,天然兼容全局变量与局部变量两种场景),不能用 `OP_get_loc`/`OP_put_loc` 配合 `define_var` 的返回值——后者只在**保证非全局**的场景下才是可用的槽位号(如函数体内的局部变量)。
