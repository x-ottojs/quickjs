...
| TS-67 | Done | 100 | Pass | `quickjs.c`/`quickjs-libc.c` | `addInitializer` **完整运行**:字段初始化改写(`x = __runInitializers(this, inits, init)`)、构造函数 extraInitializers 注入(fields_init 末尾)、静态 extra(类尾/static block)、方法共享 `_instanceExtraInitializers` 语义 | RFC §D3.4 | 与真实 tsc 逐字符一致(`["extra:s1","extra:m","extra:a","extra:b"]`);修复4类真实bug(scope_level越界死循环/OP_object非数组/字段ctor误传类/方法extra覆盖prev);`accessor` 关键字仍未实现(记录) |
