...
| TS-67 | Done | 100 | Pass | `quickjs.c`/`quickjs-libc.c` | `addInitializer` **完整运行** + **`accessor` 关键字**:字段初始化改写、构造函数/静态 extra 注入、方法共享 extras、auto-accessor(backing 私有字段 + 合成 get/set + kind="accessor" 装饰) | RFC §D3.4 | 与真实 tsc 逐字符一致(字段链 + accessor 全场景);修复4类真实bug(scope_level越界/OP_object非数组/字段ctor误传类/方法extra覆盖prev)+ 扩展 prev 消费到未装饰字段/accessor(tsc 语义验证);`accessor` 消歧(后跟 ;/}/()/= 为字段名) |
