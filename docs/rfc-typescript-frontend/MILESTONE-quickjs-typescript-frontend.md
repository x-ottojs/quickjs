...
- **`using` 声明（B2，2026-07-31，TS 5.2 显式资源管理）**：注册 `Symbol.dispose`/`Symbol.asyncDispose` well-known symbols（quickjs-atom.h + Symbol 构造器静态属性）；`using a = expr;` 编译为**原生 try/finally**（不引入 tsc 的 __addDisposableResource/__disposeResources helper）——try 内初始化 + 后续语句，finally 内 `value[Symbol.dispose]()`，多资源**嵌套 try 天然 LIFO**（后声明先释放）。异常路径：catch 入口 gosub finally 后 rethrow。与真实 tsc 逐字符一致（含异常路径）。块作用域：变量 const 声明于新 scope，块结束即 dispose。已知差异：null 值抛 TypeError（tsc helper 静默跳过——TS 类型系统禁止 null，记录）；`await using` 未实现（async disposal 需 async 上下文机制）。
